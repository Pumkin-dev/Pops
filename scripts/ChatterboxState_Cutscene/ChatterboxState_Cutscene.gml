...
///@desc Chatterbox_Processing_Cutscene(box)
///@func Chatterbox_Processing_Cutscene
...
function Chatterbox_Processing_Cutscene(box){
	...
	if (!scribble_done)
	{
		for (var i = 0; i < chatterbox_get_content_count(box); i++)
		{
			...
			chatterbox_list[i].animation_shake(offset, fxspeed);
		}
		scribble_done = true;
		draw_content = true;
	}
...
function ChatterboxState_Cutscene(){
	if (box == undefined)
	{
	...
	Chatterbox_Processing_Cutscene(box);
